...
def get_system_prompt() -> str:
    return """You are an AI agent that greets users based on the time of day. When a user asks to be greeted, use the get_greeting tool to determine the appropriate greeting and respond warmly.\n\nIMPORTANT: You MUST use the get_greeting tool to determine the correct greeting. Never fabricate, guess, or invent the time or greeting. Relay tool errors verbatim without adding suggestions."""
...
class SampleAgent:
    ...
    async def _run_agent(
        ...
        Extracted from stream() to avoid OpenTelemetry GeneratorExit issues.
        """
        with tracer.start_as_current_span("M1_user_request_received"):
            logger.info("M1.achieved: user greeting request received")

        system_prompt = get_system_prompt()
        tool_names = [tool.name for tool in tools] if tools else []
        logger.info("Running agent with %d tool(s): %s", len(tool_names), tool_names)

        with tracer.start_as_current_span("M2_time_detection"):
            logger.info("M2.achieved: current time determined")
        ...
        with tracer.start_as_current_span("M3_greeting_generated"):
            ...
            logger.info("M3.achieved: greeting generated")

        with tracer.start_as_current_span("M4_response_delivered"):
            logger.info("M4.achieved: greeting delivered to user")
    ...
    async def stream(
        ...
        try:
            ...
            logger.error("M3.missed: greeting generation failed")
            logger.error("M4.missed: response delivery failed")
            yield {
                "is_task_complete": True,
                "require_user_input": False,
